...
| Invoice cancel | `POST /api/v2/invoices/{reference}/cancel` | `POST /api/v2/invoices/{reference}/actions/cancel` | Removes the remaining Spring/OpenAPI overlap with the concrete public path `/api/v2/invoices/pay/{token}` while keeping customer payment URLs unchanged. |
