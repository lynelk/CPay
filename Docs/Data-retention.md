...
This doc was last checked against code on 2026-07-30 by reading
...
| Callback work queue | `callback_tasks`, `callback_task_claims`, `callback_delivery_signatures` | Stale `ACTIVE` rows in `callback_task_claims` are cleaned; completed `callback_tasks` and their delivery signatures are also cleaned after retention. Parked/in-flight tasks are retained for operator review. | Archive completed tasks after agreed retention |
...
| Provider run logs | `provider_sandbox_runs`, `provider_endpoint_runs`, `provider_statement_validation_runs` | Cleaned by scheduler after the configured provider-run retention window | Keep recent diagnostics without unbounded growth |
...
these independent purges/expirations in one pass:
...
2. Deletes `callback_task_claims` rows where `claim_status='ACTIVE'` and older than the configured
   retention window (cleans up claims abandoned by dead workers).
...
4. Deletes `merchant_webhook_deliveries` rows where `delivery_status` is `DELIVERED` or `FAILED` and older
   ...
   how old they are.
5. Deletes `callback_delivery_signatures` rows older than the configured signature retention window
   when the related `callback_tasks` row is already `DONE`.
6. Deletes completed `callback_tasks` rows where `task_status='DONE'` and `updated_at` is older than
   the configured callback-task retention window.
7. Deletes provider run logs from `provider_endpoint_runs`, `provider_sandbox_runs`, and
   `provider_statement_validation_runs` after the configured provider-run retention window.
8. Force-deletes `SPRING_SESSION` rows whose `CREATION_TIME` is older than the configured absolute
...
| `CPAY_CALLBACK_TASK_RETENTION_DAYS` | `30` | Deletes completed callback task rows past this age |
| `CPAY_CALLBACK_SIGNATURE_RETENTION_DAYS` | `30` | Deletes callback delivery signatures for completed tasks past this age |
| `CPAY_PROVIDER_RUN_RETENTION_DAYS` | `90` | Deletes provider endpoint, sandbox, and statement-validation run rows past this age |
...
| Callback tasks that are not completed | `callback_tasks`, `callback_delivery_signatures` | Only completed callback tasks/signatures are purged. Parked, pending, or retrying tasks are retained so operators can inspect and requeue them. | Partially enforced — completed tasks only |
...
- Decide whether long-lived `PARKED` callback tasks should be archived after an operator-visible hold
  period or retained indefinitely as incident evidence.
