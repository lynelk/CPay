...
/**
 * Covers audit F5 (extend operational purge coverage to password reset tokens and webhook
 * deliveries, which were previously left ungoverned) and E4 (an absolute session-lifetime cap -
 * Spring Session's own expiry is inactivity-only, so a session kept active indefinitely would
 * otherwise never expire).
 */
class OperationalDataCleanupSchedulerTest {

    @Test
    void purgesAllFiveOperationalDataCategories() {
        NamedParameterJdbcTemplate jdbcTemplate = mock(NamedParameterJdbcTemplate.class);
        when(jdbcTemplate.update(any(String.class), any(MapSqlParameterSource.class))).thenReturn(1);

        OperationalDataCleanupScheduler scheduler = new OperationalDataCleanupScheduler(jdbcTemplate);
        ...
        verify(jdbcTemplate).update(contains("callback_task_claims"), any(MapSqlParameterSource.class));
        verify(jdbcTemplate).update(contains("password_reset_tokens"), any(MapSqlParameterSource.class));
        verify(jdbcTemplate).update(contains("merchant_webhook_deliveries"), any(MapSqlParameterSource.class));
        verify(jdbcTemplate).update(contains("SPRING_SESSION"), any(MapSqlParameterSource.class));
        verify(jdbcTemplate, times(5)).update(any(String.class), any(MapSqlParameterSource.class));
    }
    ...
    void sessionExpiryUsesEpochMillisNotATimestamp() {
        ...
        verify(jdbcTemplate).update(contains("SPRING_SESSION"), org.mockito.ArgumentMatchers.argThat(
