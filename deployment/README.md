...
- `CPAY_ADMIN_PASSWORD` — Initial password for the `admin@example.com` super user. The deployment script will hash this password and inject it into the database seed script. It is highly recommended to set this to a secure, one-time value during the first deployment and then change the password immediately from within the application.
...
sudo CPAY_ENVIRONMENT=production \
     ...
     CPAY_ADMIN_PASSWORD='YourSecurePasswordHere!' \
...
- Before Flyway executes, the deployment flow imports the legacy SQL files in this order: `structure.sql`, `seed.sql`, and `migration_2024.sql`.
