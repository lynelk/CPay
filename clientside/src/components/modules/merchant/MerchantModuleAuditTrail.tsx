...
/**
 * Audit L2/L3/L4: the merchant-portal audit trail list - the merchant-side twin of
 * `ModuleAuditTrail`. Converted to a typed function component backed by `useMerchantAuditTrail`.
 */
...
interface MerchantModuleAuditTrailProps {
  loader?: (op: 'START' | 'STOP') => void;
  refreshSignal?: unknown;
  sessionExpired?: () => void;
}

function MerchantModuleAuditTrail({ loader, refreshSignal, sessionExpired }: MerchantModuleAuditTrailProps): React.ReactElement {
  ...
  const query = useMerchantAuditTrail(search, 50);
      ...
      <div style={{ padding: 'var(--ios-space-4)' }}>
        <Toolbar>
          <div style={{ minWidth: 200 }}>
            <Select id="merchant-audit-category" value={category} options={CATEGORIES} onValueChange={setCategory} />
          </div>
          <Toolbar.Spacer />
          <SearchField
            value={pendingValue}
