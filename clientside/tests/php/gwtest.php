<?php
    ...
    */
    ...
    $amount = "500";
    $description = "Payment..";
    $payer_account = "256701583888";//"256783086794";
    $callback_url = "";
    $reference = "260966220217-0001-271";
	...
	{
        global $base_url, $merchant_number, $description, $amount, $payer_account, $callback_url;
        $private_key_c = file_get_contents("keys/private_key.key");
	...
	{
        global $base_url, $merchant_number, $description, $amount, $payer_account, $callback_url, $reference;
        $private_key_c = file_get_contents("keys/private_key.key");
        
        $url = $base_url."doTransactionCheckStatus";
		...
		$data = $d['merchant_number'].$d['reference'];
    ...
    function initiate_send_sms() 
	{
        global $base_url, $merchant_number, $description, $amount, $payer_account, $callback_url;
        $private_key_c = file_get_contents("keys/private_key.key");
        
        $url = $base_url."doSendSms";
		
		$d=array(
			'merchant_number' => $merchant_number, 
			'recipients' => "256783086794,256701583888",
			'signature' => "", 
			'content' => "This is a single message going to all numbers.", 
		);

		$data = $d['merchant_number'].$d['content'].$d['recipients'];
    ...
    {
        $public_key_c = file_get_contents("keys/public_key.pem");
