...
CREATE INDEX idx_screening_provider_requests_subject
    on screening_provider_requests(subject_type, subject_reference);
CREATE INDEX idx_screening_provider_requests_status
...
create table if not exists cross_border_payout_rail_dispatches (
    ...
    destination_amount DECIMAL(19, 4),
...
CREATE UNIQUE INDEX uq_cross_border_payout_dispatch_idempotency
    on cross_border_payout_rail_dispatches(idempotency_key);
CREATE INDEX idx_cross_border_payout_dispatch_transfer
...
create table if not exists settlement_posting_runs (
    ...
    currency varchar(3),
    expected_total DECIMAL(19, 4),
    posted_total DECIMAL(19, 4) not null default 0,
...
CREATE UNIQUE INDEX uq_settlement_posting_runs_batch
    on settlement_posting_runs(settlement_batch_type, settlement_batch_id);
CREATE INDEX idx_settlement_posting_runs_batch
...
CREATE INDEX idx_settlement_posting_entries_run
...
CREATE INDEX idx_production_maturity_validation_results_run
