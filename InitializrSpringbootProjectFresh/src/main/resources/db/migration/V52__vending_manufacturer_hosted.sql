...
CREATE TABLE IF NOT EXISTS `vending_connector_configs` (
  ...
  `callback_heartbeat_value` VARCHAR(120) NOT NULL DEFAULT 'HEARTBEAT',
  `callback_return_value` VARCHAR(120) NOT NULL DEFAULT 'ASSET_RETURNED',
  `callback_release_value` VARCHAR(120) NOT NULL DEFAULT 'ASSET_RELEASED',
  `callback_offline_value` VARCHAR(120) NOT NULL DEFAULT 'DEVICE_OFFLINE',
...
-- Existing vending devices already carry a nullable public_token. Phase 2 makes it operational and
