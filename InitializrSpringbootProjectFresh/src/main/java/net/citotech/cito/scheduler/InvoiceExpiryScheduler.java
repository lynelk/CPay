...
/**
 * Auto-expires DRAFT/SENT invoices (audit N9) whose due_date has passed, so an invoice doesn't
 * stay payable indefinitely just because nobody ever opened its pay link -
 * {@code InvoiceService} only lazily flips an overdue invoice to EXPIRED when its pay link *is*
 * opened; this sweep covers the ones that never are. Purely a status flip on an additive column,
 * so - like {@link OperationalDataCleanupScheduler} - it is non-destructive and enabled by default.
 */
@Component
public class InvoiceExpiryScheduler {
    ...
    public void expireOverdueInvoices() {
        ...
        try {
            ...
            if (expired > 0) {
                logger.log(Level.INFO, "Invoice expiry sweep flipped {0} overdue invoice(s) to EXPIRED", expired);
            }
        } catch (Exception ex) {
