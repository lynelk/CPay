...
public class StartupApplicationListener {
    ...
    private void validateGatewayConfig() {
        try {
            net.citotech.cito.Model.Setting state =
            ...
            String[] requiredKeys = {
                "gw_mtn_api_url", "gw_airtelmoney_api_url", "gw_safaricom_api_url"
            };
            for (String key : requiredKeys) {
                ...
                if (s == null || s.getSetting_value().trim().isEmpty()) {
                    java.util.logging.Logger.getLogger(TransactionsLogController.class.getName())
                            .log(
                                    Level.SEVERE,
                                    "STARTUP CONFIG ERROR: setting '"
                                            + key
                                            + "' is required in production but is blank or missing.",
                                    "");
                }
            }
        } catch (Exception ex) {
            java.util.logging.Logger.getLogger(TransactionsLogController.class.getName())
                    .log(
                            Level.WARNING,
                            "Could not validate gateway config at startup: " + ex.getMessage(),
                            ex);
        }
    }
        ...
        try {
            if (!legacyDbChangesEnabled) {
                java.util.logging.Logger.getLogger(TransactionsLogController.class.getName())
                        .log(
                                Level.INFO,
                                "Legacy DB change XML runner disabled; Flyway is the canonical migration path.");
                return "Disabled";
            }
            ...
            if (!lockHeld) {
                java.util.logging.Logger.getLogger(TransactionsLogController.class.getName())
                        .log(
                                Level.WARNING,
                                "Legacy DB change XML runner skipped; another instance holds the startup migration lock.");
                return "Locked";
            }
            if (resources == null || resources.length == 0) {
                java.util.logging.Logger.getLogger(TransactionsLogController.class.getName())
                        .log(
                                Level.INFO,
                                "No DB change XML resources found; skipping startup DB application.");
                return "Skipped";
            }
            InputStream resource;
            for (final Resource res : resources) {
                String file =
                        Common.CLASS_PATH_GENERAL_DBCHANGES_DIR
                                + File.separator
                                + res.getFilename();
    ...
    private boolean acquireStartupMigrationLock() {
        ...
        Number locked =
                jdbcTemplate.queryForObject(
                        "SELECT GET_LOCK(:lock_name, :timeout_seconds)", parameters, Number.class);
    ...
    private void releaseStartupMigrationLock() {
        try {
            ...
            parameters.addValue("lock_name", "cpay:legacy-dbchanges");
            jdbcTemplate.queryForObject(
                    "SELECT RELEASE_LOCK(:lock_name)", parameters, Number.class);
        } catch (Exception ex) {
            java.util.logging.Logger.getLogger(StartupApplicationListener.class.getName())
                    .log(
                            Level.WARNING,
                            "Failed to release legacy DB changes startup lock: " + ex.getMessage(),
                            ex);
        }
    }
        ...
        try {
            ...
            for (int temp = 0; temp < nList.getLength(); temp++) {
                ...
                if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if (eElement.getElementsByTagName("Sql").getLength() > 0) {
                        sql = eElement.getElementsByTagName("Sql").item(0).getTextContent().trim();
                    }
                    if (eElement.getElementsByTagName("RollBack").getLength() > 0) {
                        rollback =
                                eElement.getElementsByTagName("RollBack")
                                        .item(0)
                                        .getTextContent()
                                        .trim();
                    }
            ...
            for (int i = 0; i < queries.size(); i++) {
                ...
                if (nQuery.getQuery_id() != null && !nQuery.getQuery_id().isEmpty()) {
                    ...
                    RowMapper<QueryUpdate> rm =
                            new RowMapper<QueryUpdate>() {
                                public QueryUpdate mapRow(ResultSet rs, int rowNum)
                                        throws SQLException {
                    ...
                    List<QueryUpdate> listQueries = jdbcTemplate.query(checkSql, checkParams, rm);
                    if (listQueries.size() > 0) {
                        java.util.logging.Logger.getLogger(
                                        TransactionsLogController.class.getName())
                                .log(
                                        Level.INFO,
                                        "Query already applied: " + nQuery.getQuery_id(),
                                        "");
                        continue;
                    }
                    // Now go ahead and apply this query.
                    try {
                        PreparedStatementCallback<Boolean> action =
                        ...
                        for (int j = i; j >= 0; j--) {
                            ...
                            if (q) {
                                java.util.logging.Logger.getLogger(
                                        ...
                                        .log(
                                                Level.INFO,
                                                "Executed Rollback " + nQueryR.getRollback(),
                                                ex);
                            }
                        }
                        return false;
