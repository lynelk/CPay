 ...
 * response body directly into a field that reaches merchant-facing responses on both API
 * generations: v1 via {@code GeneralException}/{@code GeneralSuccessResponse#getApiTxMessage}
 * (nested {@code txDetails.message}), and v2 via {@code AdapterNativePaymentService} / {@code
 * NativePaymentsV2Controller} ({@code PaymentResult.message} and {@code .providerResponse}). That
 * is a real, currently-reachable leak, not a theoretical one - traced end to end while building
 * this class; see the wired call sites and their tests for the exact path.
 *
 * <p>Separately (audit J7), some of those same catch blocks handed the raw internal exception
 * message (a {@code JSONException}, {@code IOException}, encryption failure, ...) straight to the
 * merchant field too, with no log entry at all - the cause was neither safely surfaced nor actually
 * logged. {@link #translateInternalFailure(Throwable)} covers that case: a short, stable,
 * non-sensitive reason code for the response, while the caller is expected to log the full
 * exception for internal diagnosis and keep the raw provider/exception detail in {@code
 * GateWayResponse#requestTrace} - a field that already exists specifically for this (it is stored
 * internally as {@code tx_request_trace}/{@code tx_update_trace} and is never serialized into a
 * merchant-facing response - see {@code GeneralException/GeneralSuccessResponse#getApiTxMessage}
 * and {@code PaymentResult}, neither of which include it).
 *
 ...
 * mean for the legacy numeric codes registered in {@link ErrorCatalog}, and {@code
 * PROVIDER_DECLINED}/ {@code PROVIDER_TIMEOUT} are the same stable codes already documented in
 * {@code Docs/Error-catalog.md}.
 *
 ...
 *
 * <ol>
 *   <li>Call {@link #translateProviderResponse(int, String, String)} (HTTP-style provider failures)
 *       or {@link #translateInternalFailure(Throwable)} (a caught internal exception), and use
 *       {@code translation.merchantMessage()} for the merchant-facing message field.
 *   <li>Keep the raw text only in {@code GateWayResponse#requestTrace} (or the logger) - never in
 *       {@code message}.
 * </ol>
 *
 * See {@code AirtelMoneyOpenApiPaymentGateway.submit()}/{@code applySuccessResponse()}, {@code
 * ProviderEndpointExecutionService#execute}, and {@code ProviderEndpointClient#execute} for worked
 * examples. {@code AirtelMoneyPaymentGateway} (classic Airtel XML gateway), {@code
 * MTNMoMoPaymentGateway}, and {@code SafariComPaymentGateway} have the identical raw-passthrough
 * pattern (`gwResponse.setMessage("Response Data: "+rs.getResponse())` and similar) and are good
 * candidates for the same mechanical change - deliberately left for a follow-up rather than
...
public final class ProviderErrorTranslator {
    ...
    /**
     * Audit C6/J7 follow-up: a response that failed our own signature verification (see {@code
     * YoPaymentsCallbackVerifier}/{@code PaymentChannelAdapter#verifyCallback}) is NOT a business
     * decline - the provider may never have sent this response at all. Deliberately a distinct,
     * non-retryable AUTHENTICATION-category entry rather than falling through to {@link
     * #translateProviderResponse}'s generic 2xx-body handling, which would otherwise collapse this
     * into indistinguishable-from-"provider declined" and lose a security-relevant signal that the
     * response could not be trusted.
     */
    public static final Translation SIGNATURE_VERIFICATION_FAILED =
            new Translation(
    ...
    // generic fallback entry (SYSTEM_UNAVAILABLE, retryable) rather than duplicating it here. The
    // key
    // is never a registered legacy numeric code, so this always hits the fallback branch.
    private static final Translation UNCLASSIFIED =
            new Translation(
     ...
     * @param providerCode a provider-specific result/error code already parsed out by the caller
     *     when available (e.g. Airtel OpenAPI's {@code status.result_code}, M-Pesa's {@code
     *     ResultCode}), or {@code null}/blank when none was parsed.
     */
    public static Translation translateProviderResponse(
            int httpStatus, String rawBody, String providerCode) {
        if (httpStatus <= 0) {
        ...
        // classified the transaction as failed via its own provider-specific business-decline
        // signal
        // (e.g. Airtel's status.result_code / transaction.status, M-Pesa's ResultCode != 0) - an
        // HTTP
     ...
     * call - not a provider HTTP error - returns a stable, non-sensitive reason so the failure is
     * not collapsed into a totally generic code with the cause visible only in the logger (or,
     * worse, with the raw exception message handed to the merchant instead). Never includes {@code
     * cause.getMessage()} or any stack trace detail in the returned message - callers are expected
     * to log {@code cause} themselves for internal diagnosis.
     */
    public static Translation translateInternalFailure(Throwable cause) {
        if (cause instanceof java.io.IOException) {
            // Reuse Category.PROVIDER: an IOException around a provider call almost always means
            // CPay
            // could not reach the provider (connect/read failure), not an internal CPay defect.
            return new Translation(
                    new Entry("PROVIDER_CONNECTION_ERROR", Category.PROVIDER, true),
                    "CPay could not reach the payment provider. Retry is safe.");
        }
        return new Translation(
                new Entry(internalReasonCode(cause), Category.SYSTEM, true),
