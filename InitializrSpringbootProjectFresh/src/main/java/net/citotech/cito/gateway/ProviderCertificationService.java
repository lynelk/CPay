 ...
 * extension adds a run-level lifecycle ({@code provider_certification_runs}) with per-run
 * scenarios and exceptions so that production channel activation can be gated on an APPROVED run
 * whose required scenarios all passed and whose blocking exceptions are resolved.
 */
@Service
public class ProviderCertificationService {
    ...
    public Map<String, Object> summary() {
        ...
        result.put("coverage", jdbcTemplate.queryForList(
            ...
            new MapSqlParameterSource()));
        result.put("runsByStatus", jdbcTemplate.queryForList(
    ...
    public long createRun(
        ...
        MapSqlParameterSource p =
                new MapSqlParameterSource()
                        ...
                        .addValue("currency", blank(currency) ? null : currency.trim().toUpperCase())
    ...
    public Map<String, Object> linkEvidence(
        ...
        MapSqlParameterSource p =
                new MapSqlParameterSource()
                        ...
                        .addValue("scenarioName", normalized(scenarioName, "UNKNOWN").toLowerCase());
        int linked =
                jdbcTemplate.update(
        ...
        int updated =
                jdbcTemplate.update(
                    ...
                    new MapSqlParameterSource()
                            ...
                            .addValue("observedStatus", blank(observedStatus) ? null : observedStatus.trim())
    ...
    public Map<String, Object> submitForReview(long runId, String submittedBy) {
        ...
        update(
            ...
            new MapSqlParameterSource()
                    .addValue("runId", runId)
                    .addValue("reviewedBy", blank(submittedBy) ? "system" : submittedBy.trim()));
    ...
    public Map<String, Object> approveRun(long runId, String approvedBy, String expiresInDays) {
        ensureRunExists(runId);
        Object status = scalar("SELECT run_status FROM provider_certification_runs WHERE id=:runId",
                new MapSqlParameterSource("runId", runId));
        ...
        if (blockers instanceof Number number && number.longValue() != 0) {
            throw new IllegalStateException("cannot approve: unresolved blocking exceptions remain");
        }
