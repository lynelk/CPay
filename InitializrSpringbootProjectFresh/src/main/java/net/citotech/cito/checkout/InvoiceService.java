 ...
 * implementation: same random-token/hash/expiry-guard shape for the public pay path, and real
 * money movement is triggered the exact same way - through
 * {@link PaymentOrchestrationService#collect(PaymentRequest, Merchant, String)} - so this class
 * never touches {@code merchant_transactions_log} or legacy pay-in logic directly.
 *
 * <p>Unlike a payment link, an invoice starts in {@code DRAFT} and is not payable until the
 * merchant explicitly {@link #send(String, String, Merchant)}s it - modeling the real-world
 * "create the invoice, review it, then send it to the customer" workflow.
 */
@Service
public class InvoiceService {
     ...
     * guards against a customer double-submitting payment while one attempt is still resolving
     * and is never exposed to callers - it is always resolved back to SENT or PAID before the
     * transaction returns.
     */
    ...
    public InvoiceResponse create(InvoiceCreateRequest request, Merchant merchant) {
        ...
        String reference = blank(request.getReference()) ? "inv_" + Common.generateUuid() : request.getReference().trim();
        ...
        try {
            jdbcTemplate.update(
                "INSERT INTO invoices "
                    ...
                    + "VALUES (:merchant_id, :reference, :amount, :currency, :description, :payer_name, :payer_contact, :due_date, '" + STATUS_DRAFT + "', :token_hash)",
                p);
        } catch (DuplicateKeyException e) {
            throw new PaymentGatewayException("An invoice with reference " + reference + " already exists for this merchant");
        }
    ...
    public InvoiceResponse send(String merchantNumber, String reference, Merchant merchant) {
        ...
        if (!STATUS_DRAFT.equals(record.getStatus())) {
            throw new PaymentGatewayException("Only a DRAFT invoice can be sent (current status: " + record.getStatus() + ")");
        }
    ...
    /** DRAFT or SENT -> CANCELLED. A PAID, EXPIRED, or already-CANCELLED invoice cannot be cancelled. */
    @Transactional
    public InvoiceResponse cancel(String merchantNumber, String reference, Merchant merchant) {
        ...
        if (!STATUS_DRAFT.equals(record.getStatus()) && !STATUS_SENT.equals(record.getStatus())) {
            throw new PaymentGatewayException("Cannot cancel an invoice with status " + record.getStatus());
        }
    ...
    /** All invoices for the merchant, newest first. The one-time pay link is not reconstructable here - see {@link #create}. */
    public List<InvoiceResponse> list(String merchantNumber, Merchant merchant) {
        validateMerchantMatches(merchantNumber, merchant);
        List<InvoiceRecord> rows = jdbcTemplate.query(
    ...
    public PaymentResult pay(String token, HostedCheckoutPayRequest payRequest, String originateIp) {
        ...
        try {
            Merchant merchant = Common.getMerchantById(String.valueOf(record.getMerchantId()), jdbcTemplate);
            if (merchant == null) {
            ...
            // existing merchant webhook system that PaymentOrchestrationService.collect(...) queues.
    ...
    private void guardPayable(InvoiceRecord record) {
        if (isOverdue(record) && (STATUS_DRAFT.equals(record.getStatus()) || STATUS_SENT.equals(record.getStatus()))) {
    ...
    private void finalizePayment(InvoiceRecord record, PaymentResult result) {
        ...
        if (STATUS_PAID.equals(status)) {
            ...
            jdbcTemplate.update(
                "UPDATE invoices SET status='" + STATUS_PAID + "', created_transaction_id=:tx_id "
                    + "WHERE id=:id AND status='" + STATUS_PROCESSING + "'",
                p);
        } else {
            // Not (yet) successful: release the reservation so the invoice can be retried.
            jdbcTemplate.update(
                "UPDATE invoices SET status='" + STATUS_SENT + "' WHERE id=:id AND status='" + STATUS_PROCESSING + "'",
                new MapSqlParameterSource("id", record.getId()));
        }
    }

    /** Maps an orchestration result onto invoice states, mirroring {@link PaymentLinkService#checkoutStatusFor}. */
    ...
    private boolean reserveForPayment(InvoiceRecord record) {
        int updated = jdbcTemplate.update(
            "UPDATE invoices SET status='" + STATUS_PROCESSING + "' WHERE id=:id AND status='" + STATUS_SENT + "'",
            new MapSqlParameterSource("id", record.getId()));
        return updated > 0;
    }
    ...
    private InvoiceResponse toResponse(InvoiceRecord record, String rawToken) {
        ...
        if (rawToken != null) {
            response.setPayUrl(appBaseUrl.replaceAll("/$", "") + "/api/v2/invoices/pay/" + rawToken);
        }
        return response;
    }
