 ...
 *
 * <p>New passwords are stored as BCrypt hashes using work factor 12. Legacy SHA-256 hex hashes are
 * still verifiable so existing accounts keep working; the hash is upgraded to BCrypt on the next
 * successful login.
 */
public final class PasswordUtils {
    ...
    public static boolean verifyPassword(String rawPassword, String storedHash) {
        ...
        if (isLegacySha256Hash(storedHash)) {
            return MessageDigest.isEqual(
                    sha256Hex(rawPassword).getBytes(StandardCharsets.US_ASCII),
    ...
    /**
     * Returns {@code true} when the stored hash is a legacy SHA-256 value that should be upgraded
    ...
    /** Returns true when an existing BCrypt hash uses a lower work factor than the current policy. */
    public static boolean needsRehash(String storedHash) {
        if (storedHash == null) {
            return false;
        }
        return isLegacySha256Hash(storedHash)
                || (isBcryptHash(storedHash) && BCRYPT.upgradeEncoding(storedHash));
    }
