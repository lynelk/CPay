...
/**
 * Covers audit K1: Testcontainers-based DB integration testing. Everything else covering
 * {@link DoubleEntryLedgerService} ({@code DoubleEntryLedgerServiceTest}) mocks
 * {@code NamedParameterJdbcTemplate}, which proves the service's own logic but never proves the
 * SQL it emits is actually correct against a real MySQL schema (correct column names, working
 * {@code ON DUPLICATE KEY UPDATE} semantics, a real unique constraint enforcing idempotency).
 * This runs the full Flyway migration history (V1..current head) against a real MySQL 8
 * container and exercises the ledger's core documented invariant (see
 * {@code Docs/Testing-strategy.md}: "balanced ledger debits/credits") plus its idempotent-post
...
class DoubleEntryLedgerServiceTestcontainersTest {
    ...
    void postsBalancedEntriesAndTheyAreVisibleInTheRealSchema() {
        ...
        List<Map<String, Object>> entries = jdbcTemplate.getJdbcTemplate().queryForList(
            "SELECT entry_direction, amount FROM ledger_entries WHERE ledger_transaction_id = " + txId
    ...
    void repostingTheSameTransactionReferenceDoesNotDuplicateEntriesInTheRealSchema() {
        NamedParameterJdbcTemplate jdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
        DoubleEntryLedgerService service = new DoubleEntryLedgerService(jdbcTemplate);
        List<LedgerEntryCommand> entries = List.of(
            entry("merchant:1002:UGX:collections_payable", "CR", "1000"),
            entry("provider:mtn_momo:UGX:float", "DR", "1000"));

        long firstPost = service.post("TX-K1-IDEMPOTENT-1", "PAYMENT", "PAY-K1-2", "first attempt", entries);
        long secondPost = service.post("TX-K1-IDEMPOTENT-1", "PAYMENT", "PAY-K1-2", "retried delivery", entries);

        assertThat(secondPost).isEqualTo(firstPost);
        Integer entryCount = jdbcTemplate.getJdbcTemplate().queryForObject(
            "SELECT COUNT(*) FROM ledger_entries WHERE ledger_transaction_id = " + firstPost, Integer.class);
