 ...
 *   backing this is static (see MTNMoMoPaymentGateway.TOKEN_REFRESH_LOCKS), not an instance field,
...
class MTNMoMoPaymentGateway401RefreshTest {
    ...
    void retriesExactlyOnceWithAFreshTokenAfterA401() throws Exception {
        ...
        server.createContext("/collection/token/", exchange -> {
            byte[] body = "{\"access_token\":\"fresh-token\"}".getBytes();
            exchange.sendResponseHeaders(200, body.length);
            exchange.getResponseBody().write(body);
            exchange.close();
        });
        server.createContext("/collection/v1_0/requesttopay", exchange -> {
        ...
        try {
            ...
            MTNMoMoPaymentGateway gateway = newGateway(
                    "http://localhost:" + server.getAddress().getPort());

            GateWayResponse response = gateway.doPayIn(1000.0, "256770000000", "ref-1", "narrative");
    ...
    private static MTNMoMoPaymentGateway newGateway(String baseUrl) {
        ...
        gateway.setApiDetails(baseUrl, "collections-user", "collections-key", "collections-subscription",
                "disbursements-user", "disbursements-key", "disbursements-subscription", "sandbox", "EUR");
