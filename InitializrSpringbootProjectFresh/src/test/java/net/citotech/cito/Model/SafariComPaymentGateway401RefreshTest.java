...
/**
 * Covers audit C2 for SafariComPaymentGateway: a token our own DB-backed store still considers
 * valid can be rejected by the provider (revoked early, clock skew, provider-side session
...
class SafariComPaymentGateway401RefreshTest {
    ...
    void singleFlightRefreshesTokenOnlyOnceUnderConcurrent401s() throws Exception {
        ...
        try {
            ...
            SafariComPaymentGateway gatewayA = newGateway(baseUrl);
            SafariComPaymentGateway gatewayB = newGateway(baseUrl);
