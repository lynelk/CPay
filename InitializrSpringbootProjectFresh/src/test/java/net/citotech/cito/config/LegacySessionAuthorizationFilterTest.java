...
class LegacySessionAuthorizationFilterTest {
    ...
    void oneFinanceSessionAllowsBillingButRejectsCommunication() throws ServletException, IOException {
        ...
        MockHttpServletRequest communication =
                new MockHttpServletRequest("GET", "/api/v2/merchant-self-service/communication/ussd/sessions");
    ...
    @Test
    void maximumAuthorityFallbackSessionCanReachAllMerchantModules() throws ServletException, IOException {
        MerchantUser legacyOwner = merchantUser(null);
        String[] paths = {
        ...
        for (String path : paths) {
            ...
            request.getSession(true).setAttribute("merchantUser", legacyOwner);
            MockHttpServletResponse response = new MockHttpServletResponse();
            MockFilterChain chain = new MockFilterChain();
            filter.doFilter(request, response, chain);
