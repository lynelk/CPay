...
/**
 * Covers {@link ProviderCostOutboxHandler}'s rate-then-persist path against {@code
 * charge_type='PROVIDER_COST'} and its no-price-book no-op, mirroring {@code
 * RatedChargeOutboxHandlerTest} for the customer-charge side.
 */
class ProviderCostOutboxHandlerTest {

    @Test
    void supportsMatchesBothPaymentDirectionsOnly() {
        ProviderCostOutboxHandler handler =
                new ProviderCostOutboxHandler(
    ...
    @Test
    void handlePersistsAProviderCostWhenAPriceBookResolves() {
        ...
        RatedCharge ratedCost =
                new RatedCharge(2L, new BigDecimal("80.00"), "UGX", "HALF_UP_SCALE_2", "[]", "{}");
        ...
        when(ratingEngine.rate(
                        ...
                        eq("UGX"),
                        eq(createdAt)))
                .thenReturn(Optional.of(ratedCost));

        new ProviderCostOutboxHandler(ratingEngine, ratedChargeRepository)
                .handle(entry(createdAt, "1000"));
        ...
        verify(ratedChargeRepository)
                .insertIfAbsent(
                        ...
                        eq(new BigDecimal("1000")),
                        eq(ratedCost),
                        eq("rated-cost:PAYMENT_COLLECTION_SUBMITTED:TX-1"));
    }

    @Test
    void handleIsANoOpWhenNoProviderCostPriceBookResolvesYet() {
        RatingEngine ratingEngine = mock(RatingEngine.class);
        RatedChargeRepository ratedChargeRepository = mock(RatedChargeRepository.class);
